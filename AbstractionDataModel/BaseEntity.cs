...
/// </summary>
/// <returns></returns>
public IQueryable<BaseEntity> GetDbSet()
{
   return new ApplicationDbContext().GetDbSet(GetType().Name);
}
...
{
   foreach (var nav in EntityUtils.GetNavigation(GetType()))
   {
       Join(nav.Name);
   }
   return this;
...
public BaseEntity Join(string propertyName)
{
   object value = null;
   if (CoreTyping.IsCollectionType(GetType().GetProperty(propertyName).PropertyType))
   {
       string entity = CoreTyping.ParseCollectionType(GetType().GetProperty(propertyName).PropertyType);
       using (var db = new CommonDataModel())
       {

           var nav = EntityUtils.GetNavigationKeyFor(entity, this.GetType());
           string p = nav.Name + "ID";
           var q = new ApplicationDbContext().GetDbSet(entity);
    ...
    /* private List<string> GetQualifiedRelationsWith(Func<Type, bool> filter)
     {
         ...
         Func<INavigation, bool> IsDictionary = (nav) =>
         {
             ...
             if (CoreTyping.IsCollectionType(propertyType))
             {
                 //TODO:
                 return false;
             }
     ...
     public object Join(string propertyName)
     {
         ...
         if (Typing.IsCollectionType(GetType().GetProperty(propertyName).PropertyType))
         {
             ...
             using (var db = new ApplicationDbContext())
             {
                 ...
                 string p = nav.Name + "ID";
                 var q = ((IQueryable<BaseEntity>)(db.GetDbSet(entity)));
                 q = q.Where(item => ReflectionService.GetValueFor(item, p).ToString() == this.ID.ToString());
                 value = q;
                 //value = (from item in ((IQueryable<BaseEntity>)(db.GetDbSet(entity))) where ReflectionService.GetValueFor(item, p).ToString() == this.ID.ToString() select p).ToList();
                 //(from p in list where ReflectionService.GetValueFor(p, zz) select p).ToList();
             }

             Setter.SetValue(this, propertyName, new object[0]);
         }
         else
         {
             object pid = GetValue(propertyName + "ID");
